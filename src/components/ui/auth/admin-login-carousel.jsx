...
import { Navbar } from "../shared/mini-navbar";
import LoginCarousel from "./login-carousel";
import { adminSigninApi, adminSignupApi } from "../../../api/authApi";

const AdminLoginCarousel = () => {
  ...
  const SigninSubmitHandler = async (e) => {
    e.preventDefault();
    try {
      const res = await adminSigninApi(Signinformdata);
  ...
  const SignupSubmitHandler = async (e) => {
    e.preventDefault();
    try {
      const res = await adminSignupApi(Signupformdata);
      localStorage.setItem("flowerToken", res.data.token);
        ...
        <div className="auth-form-panel">
          <div className="auth-form-box">
            ...
            {isSignIn ? (
              /* ─ Sign In ─ */
              <>
                <h2 className="auth-heading">Welcome Admin</h2>
...
export default AdminLoginCarousel;
